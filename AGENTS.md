...
- The public Emoji Sheet Cutter tool at `/cutter` is discoverable and must remain represented in the static sitemap, `public/agents.md`, and `public/llms.txt` while the route exists. It must keep image processing client-side unless a future change explicitly introduces and documents a server upload flow. Shared Cutter sheet assets belong on the `data` branch under `public/cutter/**`; the UI reads `public/cutter/library.json` from that branch.
