"use client";
// Source aligned with https://beui.dev/components/motion/input
...
export const Input = forwardRef<HTMLInputElement, InputProps>(function Input(
  ...
  return (
    <div className={cn("flex flex-col gap-1.5", className, classNames?.root)}>
      ...
      >
        {leftIcon ? (
          <span className={cn("pointer-events-none absolute left-3 top-1/2 flex -translate-y-1/2 items-center text-muted-foreground [&_svg]:h-4 [&_svg]:w-4", classNames?.leftIcon)}>
            {leftIcon}
          </span>
        ) : null}
        ...
        {success ? (
          <motion.svg viewBox="0 0 24 24" fill="none" className={cn("absolute right-3.5 top-1/2 h-5 w-5 -translate-y-1/2 text-(--color-success)", classNames?.successIcon)}>
            <motion.path
          ...
          <span className={cn("absolute right-0 top-0 flex h-full items-center text-muted-foreground [&_button]:grid [&_button]:size-11 [&_button]:place-items-center [&_svg]:h-4 [&_svg]:w-4", classNames?.rightIcon)}>
            {rightSlot}
          </span>
        ) : null}
      </div>
